...
nb = length(B);
%% 
% Saída do modelo sem perturbações: $x\left(t\right)=\frac{B\left(q^{-1} 
% \right)}{A\left(q^{-1} \right)}$

x = filt([0 B],A,Ts);
%% 
% Perturbações do sistema: $n\left(t\right)=\frac{C\left(q^{-1} \right)}{D\left(q^{-1} 
% \right)}e\left(t\right)$
